...
function HealOcelot(Character_1, Animal_1, Dialogue_1, Audio_1){
	...
	{
		if (keyboard_check_released(ord("Z")) && Animal_1.sprite_index == spr_dead_ocelot)
		{
			Animal_1.sprite_index = spr_alive_ocelot; //change sprite
			total_points += 1;
			animalsControl -= 1;
		...
		//talks to the animal
		if (keyboard_check_released(ord("Z")) && Animal_1.sprite_index == spr_alive_ocelot) //the animal needs to be alive
		{	
		...
		//close textbox bar
		if (keyboard_check_released(ord("X")) && Animal_1.sprite_index == spr_alive_ocelot && hide_dialogue_bar == false)
		{
