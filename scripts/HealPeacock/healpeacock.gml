...
function HealPeacock(Character_1, Animal_1, Dialogue_1, Audio_1){
	...
	{
		if (keyboard_check_released(ord("Z")) && Animal_1.sprite_index == spr_dead_peacock)
		{
			Animal_1.sprite_index = spr_alive_peacock; //change sprite 
			total_points += 1;
			animalsControl -= 1;
		...
		//talks to the animal
		if (keyboard_check_released(ord("Z")) && Animal_1.sprite_index == spr_alive_peacock) //the animal needs to be alive
		{	
		...
		//close textbox bar
		if (keyboard_check_released(ord("X")) && Animal_1.sprite_index == spr_alive_peacock && hide_dialogue_bar == false)
		{
