...
function KillsHunter(Character_1, Hunter_1, Dialogue_1, Audio_1, Camera_1){
	...
	{
		//KILLS IF NOT KILLED YET
		if (keyboard_check_released(ord("C")) && Hunter_1.sprite_index == spr_hunter_idle)
		{
			Hunter_1.sprite_index = spr_hunter_dead; //CHANGE SPRITE TO DEAD
			total_points += 2; //INCREASE PLAYER TOTAL POINTS
			huntersControl -= 1; //DECREASE TOTAL HUNTERS
			audio_play_sound(Audio_1, 10, false); //PLAY AUDIO THAT KILLS HUNTERS
		}
	
		//TALKS TO THE HUNTER WHEN ALREADY DEAD
		if (keyboard_check_released(ord("C")) && Hunter_1.sprite_index == spr_hunter_dead)
		{	
		...
		if (keyboard_check_released(ord("X")) && Hunter_1.sprite_index == spr_hunter_dead && hide_dialogue_bar == false)
		{
